        ...
        else
        {
            ...
            [globalobj GlobalDict:urlstring Globalstr:@"array" Withblock:^(id result, NSError *error) {
               ...
               //     NSLog(@"jsonResult... : %@",jsonResult);
                ...
                NSLog(@"jsonResult count... %lu",(long)[jsonResult count]);
                
                resultCount=(int)[jsonResult count];
...
-(void)btnCreate
{
    ...
    for (int i=0; i<rows; i++)
    {
        ...
        {
            [temp1 setTitle:[[jsonResult objectAtIndex:(titleCount)]valueForKey:@"category_name"] forState:UIControlStateNormal];
        }
     ...
     //   NSLog(@"Button title: %@",[[[jsonResult valueForKey:@"infoaray"] objectAtIndex:(titleCount) ]valueForKey:@"category_name"]);
        ...
        if(i!=oddRow-1)
        {
            ...
            {
                [temp2 setTitle:[[jsonResult objectAtIndex:(titleCount) ]valueForKey:@"category_name"] forState:UIControlStateNormal];
            }
    ...
    [UIView transitionWithView:sidemenu
                    ...
                    completion:^(BOOL finished)
     {
         ...
         else if (sender.tag==7)
         {
             ...
             DashboardViewController *obj=[self.storyboard instantiateViewControllerWithIdentifier:@"WishlistViewControllersid"];
